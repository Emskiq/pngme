...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        if value.len() < 8 {
            Err("At least 8 HEADER Bytes should be provided!")
        }
        else {
            let mut data = value;

            let png_header = read_header(&mut data);
            if png_header != Png::STANDARD_HEADER {
                return Err("Header bytes are incorrect!");
            }

            let mut png_chunks : Vec<Chunk> = Vec::new();
            while !data.is_empty() {
                match read_chunk(&mut data) {
                    Ok(chunk) => png_chunks.push(chunk),
                    Err(_) => return Err("Error while parsing chunks!"),
                };
            }

            Ok(Png { header: png_header, chunks: png_chunks })
        }
    }
}
...
fn read_header(input: &mut &[u8]) -> [u8; 8] {
    let (header_bytes, rest) = input.split_at(Png::HEADER_SIZE);
    *input = rest;
    header_bytes.try_into().unwrap()
}

fn read_chunk(data: &mut &[u8]) -> Result<Chunk, ParseChunkError> {
    let read_chunk: Result<Chunk, _> = (*data).try_into();

    match read_chunk {
        Ok(chunk) => {
            let (_, rest) = data.split_at(chunk.length() as usize + CRC_SIZE + CHUNK_TYPE_SIZE + CHUNK_LEN_SIZE);
            *data = rest;
            return Ok(chunk);
        }
        Err(_) => return Err(ParseChunkError),
    };
}
